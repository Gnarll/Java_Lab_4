...
// Перо для рисования графика
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, new float[] {4, 1, 1, 2, 2}, 0.0f);
...
// Отобразить график
    ...
    protected void paintGrid(Graphics2D canvas)
    {
        ...
        {
            double temp = getSize().getWidth();
            Point2D.Double center = xyToPoint(0, currentY);
            canvas.draw(new Line2D.Double(shiftPoint(center, -temp, 0), shiftPoint(center, temp, 0)));
        }
        ...
        {
            double temp = getSize().getHeight();
            Point2D.Double center = xyToPoint(currentX, 0);
            canvas.draw(new Line2D.Double(shiftPoint(center, 0, -temp), shiftPoint(center, 0, temp)));
